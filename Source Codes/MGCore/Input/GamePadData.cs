...
namespace MGCore
{
	public class GamePadData
	{
		...
		GamePadTriggers Vtriggers;
		GamePadButtons Vbuttons;
		GamePadDPad VdPad;
		public void UpdateViaVirtualSticks(DualTouchStick virtStick  )
		{
		
			_previousState = _currentState;
			GamePadThumbSticks sticks = new GamePadThumbSticks(virtStick.LeftStick.Direction, DualTouchStick.OneStick ? Vector2.Zero: virtStick.RightStick.Direction);
			_currentState = new GamePadState(sticks, Vtriggers, Vbuttons, VdPad);
		...
		public bool IsRightStickRightPressed(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.X > deadzone && _previousState.ThumbSticks.Right.X < deadzone;
		}


		public bool IsRightStickRightReleased(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.X < deadzone && _previousState.ThumbSticks.Right.X > deadzone;
		}


		public bool IsRightStickUpPressed(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.Y > deadzone && _previousState.ThumbSticks.Right.Y < deadzone;
		}


		public bool IsRightStickUpReleased(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.Y < deadzone && _previousState.ThumbSticks.Right.Y > deadzone;
		}


		public bool IsRightStickDownReleased(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.Y > -deadzone && _previousState.ThumbSticks.Right.Y < -deadzone;
		}


		public bool IsRightStickDownPressed(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.Y < -deadzone && _previousState.ThumbSticks.Right.Y >-deadzone;
		}


		public bool IsRightStickLeftReleased(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.X > -deadzone && _previousState.ThumbSticks.Right.X < -deadzone;
		}


		public bool IsRightStickLeftPressed(float deadzone = Input.DEFAULT_DEADZONE)
		{
			return _currentState.ThumbSticks.Right.X < -deadzone && _previousState.ThumbSticks.Right.X > -deadzone;
		}
