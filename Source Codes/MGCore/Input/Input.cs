...
//using Vector2 =Microsoft.Xna.Framework;
//using Point = Farseer.Xna.Framework.Point;

//TODO this is a pain , using our our vector2.  TODO see 
//how other monogame clients deal with this, or how used in Nez, Meextend.Aether , etc
// we could strip out our datacontract serialize and chage to something standard,
//bodytemplate and joint state templates
...
namespace MGCore
{
	...
	{
	/// <summary>
	/// Emitter of UI Events, adapted from Nez
	/// </summary>
		...
		internal static Vector2 _resolutionScale = Vector2.One;
		...
		static public DualTouchStick DualTouchStick;


		static public bool UseVirtSticks = true;
		...
		/// </summary>
		//public static Vector2 ResolutionScale => _resolutionScale;  MG_GRAPHICS commented , seems related to pixel perfect sutff, set in Scene, TODO erase if never needed
		...
		public static int MaxSupportedGamePads
		{
			...
			set
			{
				...
				for (var i = 0; i < _maxSupportedGamePads; i++)
					GamePads[i] = new GamePadData((Microsoft.Xna.Framework.PlayerIndex) i);
			}
		}


		/// <summary>
		/// Events on changes.  Since we wil have only one listener , no need to use the Emitter
		/// </summary>
		//	public static Action<Keys> KeyDown;
		//	public static Action<Keys> KeyUp;
		//	public static Action<MouseState> MouseDown;
		//	public static Action<MouseState> MouseUp;
		//	public static Action<MouseState> MouseMouse;

		public static bool IsTouchSticksEnabled
        {
			get { return DualTouchStick != null && DualTouchStick.IsEnabled; }
			set
			{
				if (DualTouchStick != null)
				{ DualTouchStick.IsEnabled = value; }
			}			
        }

		static public void InitVirtualSticks(SpriteFont font, ContentManager cm)
		{

			if ( !Input.Touch.IsConnected)
            {
				Debug.WriteLine("touch not connected");
				return;
            }


			if (Input.GamePads.Length > 0 && Input.GamePads[0]!=null && Input.GamePads[0].IsConnected()) 
			{
				Debug.WriteLine("Gamepad connected, not using touch stick");
				return;
			}


			DualTouchStick = new DualTouchStick(font, cm, 1.3f,0.05f,25f,60,30);
			DualTouchStick.LeftStick?.SetAsFree();
			DualTouchStick.RightStick?.SetAsFree();
		...
		static public bool SticksReady()
        {
			return DualTouchStick != null;
		}
		...
		public static void Update()
		{
			...
			if (DualTouchStick != null)
			{
				DualTouchStick.Update(1000 / 60);//TODO does it need the real dt
				Input.GamePads[0].UpdateViaVirtualSticks(DualTouchStick);
				return;
			}
