...
{
    //needed to add 2 collections, since List<TPoint> wont cast to  PointCollection in graphics 
    //TCollection could be a direct graphics collection list of TPoint, sent to a polygon, using doubles

        //or use for physics, send in float, and List<float>.. pretty sure doubles are faster in 64 bit machines
        //anyways
    public class VisibilityPolygonCSharp<TPoint> where  TPoint : struct
  {
    ...
    public void Compute(TPoint position, List<Segment<TPoint>> segments,  IList<TPoint>  collection)
    {
      if (segments == null)
      {
      ...
      var polygon = collection //   new List< new TCollection();
