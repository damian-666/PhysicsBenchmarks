...
namespace FarseerPhysics.Dynamics.Joints
{
    ...
    /// </summary>
    /// 
    [DataContract(Name = "PrismaticJoint", Namespace = "http://ShadowPlay", IsReference = true)]
    public class PrismaticJoint : Joint
    {

        //Note.. issues  motor does work.. no reference angle.. difficult to use .. changing one limits messes up the other .. though they work.
        //Diffed with the 3.5 code.. due to  new solver data passed in through SolveVelocityConstraints, drop in/ compile is impractical,  d  and  LocalXAxis1  still is seem to be report in world, bad for modelling/ serialization
        [DataMember]
        ...
        // [DataMember]  the desired motor speed.   Doest seem to work anyways
        ...
        public float MotorForce
        {
          ...
          //  set { _motorImpulse = value; } //shadow play mod.. this appears to be determined by motorSpeed.. should not need a set..
        }

        // axis gets mixed up after save/ reload.. is this fixed by below.. not sure.   needs more testing 
//TODO CLEANUP figure out  removing the order breaks old files with joint .. diff to see why.  because  the order should not matter now after fix
        [DataMember (Order=99)]  // tried ordering still flips axis on reload ..needs to be saved after body A ?   or save the LocalXAxis1Y .. etc.. then set on init..
        public Vector2 LocalXAxis1
        {
            get { return _localXAxis1; }
            set
            {
                //_localXAxis1 = BodyA.GetLocalVector(value);       // probably typos.  Model is always saved in Body local
                #region ShadowPlay Mods
                _localXAxis1 = value;
                #endregion

                _localYAxis1 = MathUtils.Cross(1.0f, _localXAxis1);  // just the perpendicular
            }
        }


        #region ShadowPlay Mods:  just so we can edit in prop sheet

        public float LocalXAxis1Y
        {
            get { return _localXAxis1.Y; }
            set
            {
                _localXAxis1.Y = value;
            }
        }

        public float LocalXAxis1X
        {
            get { return _localXAxis1.X; }
            set
            {
                _localXAxis1.X = value;
            }
        }
        ...
        internal override void InitVelocityConstraints(ref TimeStep step)
        {
            ...
            else
            {
                ...
                NotifyPropertyChanged("LocalXAxis1"); 
