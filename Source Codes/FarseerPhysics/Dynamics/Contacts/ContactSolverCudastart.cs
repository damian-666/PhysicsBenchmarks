...
using FarseerPhysicsUA.Dynamics.Contacts;
...
namespace FarseerPhysics.Dynamics.Contacts
{
   ...
   // [cudafy]
    public  struct ContactConstraint
    {
        ...
        public void  Reset()
        {
            Points = new ContactConstraintPoint[Settings.MaxPolygonVertices];
    ...
    {
        public List< ContactConstraint> Constraints;
        ...
        public void Reset(Contact[] contacts, int contactCount, float impulseRatio, bool warmstarting)
        {
            ...
            Constraints = new List<ContactConstraint>(_constraintCount);
            _constraintCount = contactCount;


            // grow the array
            if (Constraints == null || Constraints.Count < _constraintCount)
            {
                Constraints = new List<ContactConstraint>(_constraintCount*2);
...
#region  ShadowPlayMod
            //   cc.Friction = Settings.MixFriction(fixtureA.Friction, fixtureB.Friction);
                cc.Friction = Settings.MixFrictionSliding( fixtureA,  fixtureB);
#endregion
                ...
                if (c.PointCount == 1)
                {
                    ...
                    float lambda = -ccp.NormalMass*
                                   ((c.BodyB.LinearVelocityInternal.X + (-wB*ccp.rB.Y) -
                                    ...
                                    (c.BodyB.LinearVelocityInternal.Y + (wB*ccp.rB.X) - c.BodyA.LinearVelocityInternal.Y -
...
namespace FarseerPhysicsUA.Dynamics.Contacts
{
    class cudafyAttribute : Attribute
    {
    }
}