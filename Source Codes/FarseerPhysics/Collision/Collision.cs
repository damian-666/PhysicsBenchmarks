...
namespace FarseerPhysics.Collision
{
    ...
    public struct RayCastInput
    {
        ...
        #region ShadowPlay Mods used by sensor 
        public bool TestInside;
     //   public object UserData;// legacy is  not used now in tool or play
        
        public RayCastCallback Callback;        //pass the callback through, for thread safety.
    ...
    /// </summary>
    [DataContract(Name = "AABB", Namespace = "http://ShadowPlay")]
    public struct AABB
    {
        /// <summary>
        /// The lower vertex
        ...
        public AABB(ref Vector2 min, ref Vector2 max)
        {
            ...
            // fix case when aabb min max not proper
            LowerBound.X = Math.Min(min.X, max.X);
            LowerBound.Y = Math.Min(min.Y, max.Y);
            UpperBound.X = Math.Max(min.X, max.X);
            UpperBound.Y = Math.Max(min.Y, max.Y);
        ...
        /// </summary>
        /// <param name="aabb">The aabb input</param>
        ...
        {
            //using epsilon to try and guard against float rounding errors.
            if ((X > (LowerBound.X + Settings.Epsilon) && X < (UpperBound.X - Settings.Epsilon) &&
                 (Y > (LowerBound.Y + Settings.Epsilon) && Y < (UpperBound.Y - Settings.Epsilon))))
            {
                return true;
            }
            return false;
        ...
        /// <summary>
        /// AABB contains point, does not have be ref, for use with properties such as WorldCenter
        /// </summary>
        ...
        public static float  OverlapArea(ref AABB a, ref AABB b)
        {
            Vector2 d1 = b.LowerBound - a.UpperBound;
            Vector2 d2 = a.LowerBound - b.UpperBound;
           ...
           float  x = Math.Max( b.LowerBound.X ,  a.LowerBound.X ) - Math.Min( b.UpperBound.X ,  a.UpperBound.X ) ;
           float  y = Math.Max( b.LowerBound.Y ,  a.LowerBound.Y ) - Math.Min( b.UpperBound.Y,  a.UpperBound.Y ) ;
        ...
        /// <summary>
        /// Get the Overlap or intersecting AABB with this one. 
        /// </summary>
        /// <param name="aabb1">The aabb1.</param>
        /// <param name="aabb2">The aabb2.</param>
        public void Overlap( AABB aabb)
        {

            UpperBound.X = Math.Min(UpperBound.X, aabb.UpperBound.X);
            UpperBound.Y = Math.Min(UpperBound.Y, aabb.UpperBound.Y);

            LowerBound.X = Math.Max(LowerBound.X, aabb.LowerBound.X);
            LowerBound.Y = Math.Max(LowerBound.Y, aabb.LowerBound.Y);
        ...
        /// <summary>
        /// expand an AABB by a factor 
        /// </summary>
        /// <param name="aabb"></param>
        /// <param name="xFactor"></param>
        /// <param name="yFactor"></param>
        /// <returns></returns>
        public AABB Expand( float xFactor, float yFactor)
        {
            AABB expandedAABB = this;
            float xAdjust = Width * xFactor;
            float yAdjust = Height * yFactor;

            expandedAABB.UpperBound.X += xAdjust;
            expandedAABB.UpperBound.Y += yAdjust;
            expandedAABB.LowerBound.X -= xAdjust;
            expandedAABB.LowerBound.Y -= yAdjust;
            return expandedAABB;
        }


        /// <summary>
        /// Expand AABB by specified amount of space. 
        /// </summary>
        public AABB Expand(float left, float top, float right, float bottom)
        {
            AABB expandedAABB = this;
            expandedAABB.LowerBound.Y -= top;
            expandedAABB.LowerBound.X -= left;
            expandedAABB.UpperBound.Y += bottom;
            expandedAABB.UpperBound.X += right;
        ...
        public Vertices VerticesCounterclockwise  //NOTE above Vertices is clockwise, dont want to retest farseer so leaving it
        {
            get
            {
                Vertices vertices = new Vertices();
        ...
        /// <summary>
        /// Copy the contents of another AABB to this AABB. Similar to copy constructor,
        /// but we're not creating new instance here.
        /// </summary>
    ...
    public static class Collision
    {
        ...
        //PARALLEL PARALLIZATION NOTE.. IF THERE IS ANY ONE 1 PROBLEM, CAN x INTERVALS BE DONE IN PARALLEL WITH y?
                                            ...
                                            PolygonShape poly2, ref Transform xf2)
        {
            ...
            for (int i = 0; i < count2; ++i)  //TODO Parallel loop?   multithreading?
            {
                                             ...
                                             PolygonShape poly2, ref Transform xf2)
        {
            ...
            int index = 0;
            float minDot = Settings.MaxFloat;
            for (int i = 0; i < count2; ++i)   //  <------SHADOWPLAY NOTE //PARALLEL PARALLIZATION with thread pool.. can this loop go faster .. anything that walks verts could.
            {
