...
namespace FarseerPhysics.Collision
{
    ...
    public class DynamicTree<T>
    {
        ...
        public void Query(Func<int, bool> callback, ref AABB aabb)
        {
            ...
            Stack<int> stack = new Stack<int>(256);   //shadowPlay Mods .. for thread safety dont use static _stack .
            stack.Clear();
            stack.Push(_root);
        ...
        public void RayCast(RayCastCallbackInternal callback, ref RayCastInput input)
        {
            ...
            Stack<int> stack = new Stack<int>(256);   //shadowPlay Mods dh.. for thread safety dont use static _stack .
                                                      //rays are cast from plugin (UI ) and phyiscs (controller)
                                                      //box 
                                                      //cannot see any performance impact on lot of particles each casting ra.
                                                      // normall collision still uses static stack.. tested parallet ray cast thread in controller also n 800 partilce in two groups

            //todo fix normal ray cast collisoin thing and see if parallel implementation can work

            //Stack<int> stack  = _stack;      //before in old farseer.
            //TODO consider we do AABB query in spirit at same times as collision .. consider removal of _stack there as well.

            Vector2 p1 = input.Point1;
            Vector2 p2 = input.Point2;
            Vector2 r = p2 - p1;

            if (r.LengthSquared() <= 0.0f)   //shadowplay mod
            {
             			
                Debug.WriteLine("ray dist is zero");
				return;
            }
            ...
            //shadowplay Mod use stack instead of heap for parallelism
            #region ShadowPlay Mod  
            Vector2 vec = Vector2.Zero;
            vec.X = -r.Y;
            vec.Y = r.X;
            Vector2 absV = MathUtils.Abs(vec);
            #endregion 
          ...
          //  Vector2 absV = MathUtils.Abs(new Vector2(-r.Y, r.X));
            ...
            while (stack.Count > 0)
            {
                ...
                if (node.IsLeaf())
                {
                    ...
                    //dh make  a copy of the raycast RayCastInput..  I thinkt his is needed when rays are cast in our plugin UI thread
                    //at same time of ray used in physics thread, just a guess.   
                    // but clearly seen, without this, Creature ground foot detect doesnt seem to work during rain with ray.. level1raintest..
                    // no difference in FPS seen  with this copy.   //TODO check if this is needed now with our thread safe local stack
                    RayCastInput subInput;
                    subInput.Point1 = input.Point1;
                    subInput.Point2 = input.Point2;
                    subInput.MaxFraction = maxFraction;
                    subInput.Callback = input.Callback;//dh added this to pass the shape callback without using member var.. (thread safe)                     
        ...
        private void FreeNode(int nodeId)
        {
            ...
            // this should prevent memory leaks when deleting a node
