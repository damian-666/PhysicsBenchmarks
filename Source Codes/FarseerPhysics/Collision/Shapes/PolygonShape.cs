...
namespace FarseerPhysics.Collision.Shapes
{
    ...
    /// </summary>
    /// 
    [DataContract(Name = "PolygonShape", Namespace = "http://ShadowPlay", IsReference = true)]  
        ...
        public void Set(Vertices vertices)
        {
            ...
            for (int i = 0; i < vertices.Count; ++i)
            {
                ...
                Vector2 edge = Vertices[i2] - Vertices[i1];
                  ...
                  // Shadowplay Mod: throw exception when invalid edge found, so   
                 // fixture creation will be canceled for this polygon.  
                 if (edge.LengthSquared() <= Settings.Epsilon * Settings.Epsilon)  
                 {  
                     throw new Exception("PolygonShape.Set(Vertices): Invalid edge found.");  
                 }
//shadowplay mod , remove assert 
...
#if CLOUDNOTJITTERING && !PRODUCTION  // dont ever assert.. its ok.. sometimes we are using PolygonShape just for graphics model ( clouds, which can then be randomly jittered resulting in invalid polygons
//with isNotcollidable. there is not way to tell from here if it is isNotcollidable...TODO.
                     // so fixture creation will be canceled for this polygon.  
                     
                    if (s <= 0.0f)  
                     {  
                        Debug.WriteLine( "Collinear or non-convex edges found.");
                         throw new Exception("PolygonShape.Set(Vertices): Collinear or non-convex edges found.");             
                    }                   
                    Debug.Assert(s > 0.0f);
#else
                     if (s <= 0.0f)  
                     {  
                        Debug.WriteLine( "Collinear or non-convex edges found.");// dont care for clouds or non collidabe, we are not making fixtures.
                     }
#endif 
                }
            }
...
#region ShadowPlay Mod
            //The area is too small for the engine to handle.   NOTE dotsnt seem to cause a problem , dont assert
          //  Debug.Assert(area > Settings.Epsilon);
            if (area < Settings.Epsilon)
			{
            
              Debug.WriteLine("ComputeProperties:The area is too small for the engine to handle"); 
			//  throw new Exception("ComputeProperties:The area is too small for the engine to handle");  
...
#region ShadowPlay Mod//  adapted from above.. should probably call this..

        /// <summary>
        /// Test a point for containment in this shape. This only works for convex shapes.
        /// </summary>
        /// <param name="point">a point in loal coordinates.</param>
        /// <returns>True if the point is inside the shape</returns>
        protected bool TestLocalPoint(ref Vector2 point)
        {
            for (int i = 0; i < Vertices.Count; ++i)
            {
                float dot = Vector2.Dot(Normals[i], point - Vertices[i]);
                if (dot > 0.0f)
                {
                    return false;
                }
        ...
        {
#region ShadowPlay Mod
            //  output = new RayCastOutput(); //dh shadowplay mod.. avoid use of heap, doing parallel ray casts..       
            output.Fraction = 0;   //this is how its initialized.
            ...
            if (input.TestInside ) //this means if inside return a hit with fraction 0..  avoids a second broadphase query..
            {       
                 if ( TestLocalPoint( ref p1))
                 {
                     output.Fraction = 0;                  
                     return true;
                 }
        ...
        public override float ComputeSubmergedArea(Vector2 normal, float offset, Transform xf, out Vector2 sc)
        {
            ...
            //shadow play notes... this is  not documented anywhere i can find.  I cannot find a sample that has not a horizontal AABB.
           //i have to cahgne the offset passed in to negative or it didnt work.

            //the contribution was adapted 
            //Transform plane into shape co-ordinates

            //see bouyancy in the box2d contribution , its is 3D and undocumented .  the Plane is also defined as an offset and a normal
            //also the demo uses a horizontal water.
            //A plane is defined as a Point and an offset.    in math a plane is defined as  a point and an normal.
            //here is the same issue..  if we assume that the point is 0,0.    tried the offset from the body center to the surface, didnt work.
            //in here https://code.google.com/p/gmogre3d/wiki/PlaneFunctions
            //says the "depth" is defined as the distance to move the plane to the origin -along- the normal... so this can be calculated.=
            //thats how in ogre game maker planes are made..  so perhaps that is the convention..
            //however will forgo trying this.   this is not the best way.   for wavy surfaces the intersection of polygon is the best and can be optimized
          // to be cheap without much effort
            // https://code.google.com/p/gmogre3d/wiki/PlaneFunctions
