...
namespace FarseerPhysics.Factories
{
    ...
    public static class PathManager
    {
        ...
        /// <param name="body">The body.</param>
        /// <param name="subdivisions">The subdivisions.</param>
        public static List<Vector2> ConvertPathToCollisionPolygon(Path path, Body body, int subdivisions)
        {
            if (!path.Closed)
            ...
            PartitionFixturesFromVerts(body, verts);

            return verts;
        }

        public static void PartitionFixturesFromVerts(Body body, List<Vector2> verts)
        {
            ...
            {
                body.CreateFixture(new PolygonShape(item, body.Density));
            }
        }


        /// <summary>
        /// Convert a closed path into a polygon to Draw, no collision
        /// </summary>
        /// <param name="path">The path.</param>
        /// <param name="subdivisions">The subdivisions.</param>
        public static List<Vector2> ConvertPathToViewingPolygon(Path path, int subdivisions)
        {
            if (!path.Closed)
                throw new Exception("The path must be closed to convert to a polygon.");

             return path.GetVertices(subdivisions);
