...
namespace FarseerPhysics.Common
{
    ...
    public static class LineTools
    {
        ...
        //shadowplay mod  cut out of above code.. for speed of exectution
        public static Vector2 PointOnLine(ref Vector2 point, ref Vector2 lineEndPoint1,
                                                               ref Vector2 lineEndPoint2)
        {
            Vector2 v = Vector2.Subtract(lineEndPoint2, lineEndPoint1);
            Vector2 w = Vector2.Subtract(point, lineEndPoint1);
            ...
            float c1 = Vector2.Dot(w, v);
            if (c1 <= 0) return lineEndPoint1;

            float c2 = Vector2.Dot(v, v);
            if (c2 <= c1) return lineEndPoint2;

            float b = c1 / c2;
            return Vector2.Add(lineEndPoint1, Vector2.Multiply(v, b));
        ...
        /// <summary>
        ///Check if the lines segments a0->a1 and b0->b1 cross.  //Shadowplay mod.. these are segments.  The LineIntersect with  true, true, does not seem to work reliably
        ...
        //NOTE compared with latest farseer, its the same . Jeremy Bell algorithm 
        #region ShadowPlay mod   //changed to return if segment intersected or not, not needing input specification.   Tried with Sutherland clipping to collect intersections, however its still gives too many, suspect and issue with the clipping algorithm that is not understood, rahter than this method.
        //theres is another implementation in SutherlandHodgman.cs very similiar.
		/// <summary>
        /// This method detects if two line segments (or lines) intersect,
        /// and, if so, the point of intersection. Use the <paramref name="firstIsSegment"/> and
        /// <paramref name="secondIsSegment"/> parameters to set whether the intersection point
        ...
        /// no intersection is detected (there are actually
        /// infinite intersection points).
        /// original Author: Jeremy Bell  
        /// </summary>
        /// <param name="point1">The first point of the first line segment.</param>
        /// <param name="point2">The second point of the first line segment.</param>
        /// <param name="point3">The first point of the second line segment.</param>
        /// <param name="point4">The second point of the second line segment.</param>
        /// <returns>intersection if not parallel</returns>
        /// 
        public static Vector2? LineIntersect( Vector2 point1,  Vector2 point2,  Vector2 point3,  Vector2 point4,                                  
                                         out bool segmentIntersect)
        {
            ...
            if (!(denom >= -Settings.Epsilon && denom <= Settings.Epsilon))
            {
                ...
                //There is an intersection to the extended line
                point.X = point1.X + ua * b;
                point.Y = point1.Y + ua * d;
                ...
                if ( ua >= 0.0f && ua <= 1.0f)
                {
                    ...
                    if ( ub >= 0.0f && ub <= 1.0f)
                    {
                        ...
                        if (ua != 0f || ub != 0f)
                        {
                            //There is an intersection in the two segments.
            ...
            return null;  //lines are parallel or nearly
        }
