...
namespace FarseerPhysics.Common
{
    ...
    //TODO put this is a file called MathUtils (MANUALLY, in all farseer projects) .. i did a rename and i got destroyed..  the system chaged everything with Math.
    // all mixed up..  one class  to one file.

    public static class MathUtils
    {

        //TODO find and integrate  a units class with standard converters and formatters
        public const float KmhToMs = 1000f / 3600f;
        public const float KphToMph = 0.621f;
        public const float MsToMph = 1.0f / (MathHelper.KmhToMs * MathHelper.KphToMph);
        public const float MetersToMiles = 0.000621371f;

        //moved from mathhelper for XNA conflict..
        ...
        public static Vector2 Multiply( Vector2 v, ref Mat22 A)
        {
            return Multiply(ref A,  v);
        }

        public static AABB Multiply(ref Mat22 A, AABB aabb)
        {

            AABB aabbRot = new AABB(Multiply(ref A, ref aabb.LowerBound), Multiply(ref A, aabb.UpperBound));
            return aabbRot;
        }


        public static Vector2 Multiply(Vector2 v,  Mat22 A)
        {
            return Multiply(ref A, ref v);
        }

        public static Vector2 Multiply( Mat22 A, Vector2 v)
        {
            return Multiply(ref A, ref v);
        }
        ...
        public static Vector2 Multiply(Vector2 v,ref Transform T)
        {
            return Multiply(ref T, ref v);
        }
        ...
        //clockwise positive angle between  vecs , Y is down
        public static double VectorAngleYdown(Vector2 p1,  Vector2 p2)
        {
...
//#endif
        ...
        //don't overload above.. cause we often use round nums meant to be floating pt
        public static int RandomInt(int min, int max)   
        {
            return _random.Next(min, max+1);  // +1, otherwise never gives the max.
        }

        /// <summary>
        /// Odds of getting a random number between 0 and chance.   theoretically though.. it might never happen.  with repeat calls it eventually will.
        /// check if next random number is zero.
        /// </summary>
        /// <param name="chance">lower value means better chance of hit.   IsOneIn( 10)  means a One in 10 chance of True return  </param>
        public static bool IsOneIn(int chance)
        {
            int i = _random.Next(chance);
            return (i == 0);
        }


        /// <summary>
        /// gets a random value as a fraction of base.   deviation = 1 means it can be  += 1 times value..
        /// </summary>
        /// <param name="baseValue"></param>
        /// <param name="deviationFactor"></param>
        /// <returns>adjusted value, can be less than the base</returns>
        public static float GetRandomValueDeviationFactor(double baseValue, double deviationFactor)
        {
            if (deviationFactor == 0d)
                return (float)baseValue;

            double deviationAmount = Math.Abs(baseValue * deviationFactor);

            double rand = _random.NextDouble();//sometimes in debugger its always zero//
            if (rand == 0d)
            {
                _random = new Random();
            ...
            double deviation = deviationAmount;

            return (float)(baseValue - (deviationAmount * 0.5f) + (deviationAmount * _random.NextDouble()));
        }

        /// <summary>
        /// will add X factor to the base value
        /// </summary>
        /// <param name="baseValue"></param>
        /// <param name="deviationFactor"></param>
        /// <returns>a value > greater than the base</returns>
        public static float GetRandomValuePlusDeviationFactor(double baseValue, double deviationFactor)
        {
            double deviationAmount = baseValue * deviationFactor;
            return (float)(baseValue + (deviationAmount * _random.NextDouble() ));
        }
