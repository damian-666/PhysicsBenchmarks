...
namespace FarseerPhysics.Common.Decomposition
{
    ...
    public static class EarclipDecomposer
    {
        ...
        {
            return ConvexPartition(vertices, int.MaxValue, 0, Settings.MaxPolygonVertices);// shadowPlay mod break out this as a parameter so caller can choose
        }
        ...
        /// <returns></returns>
        public static List<Vertices> ConvexPartition(Vertices vertices, int maxPolys, float tolerance, float maxVertices)  // shadowPlay mod break out this as a parameter so caller can choose
        {
            if (vertices.Count < 3)
            ...
            List<Vertices> polygonizedTriangles = PolygonizeTriangles(triangulated, maxPolys, tolerance, maxVertices);// shadowPlay mod break out this as a parameter so caller can choose
        ...
        ///<param name="tolerance">The tolerance</param>
        ///<returns></returns>
        public static List<Vertices> PolygonizeTriangles(List<Triangle> triangulated, int maxPolys, float tolerance, float maxVertices)
        {
            ...
            while (notDone)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < 2*triangulated.Count; ++i, ++index)
                    {
                        ...
                        if (newP.Count > maxVertices)// shadowPlay mod break out this as a parameter so caller can choose
                            continue;
