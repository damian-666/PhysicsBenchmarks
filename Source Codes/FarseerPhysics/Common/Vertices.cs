...
namespace FarseerPhysics.Common
{
    ...
    public class Vertices : List<Vector2>
    {
        ...
        /// <summary>
        /// build a vertex collection from a vector array.  if endindex is less thatn start, will be reversed.
        /// </summary>
        /// <param name="vector2"></param>
        /// <param name="startIndex"></param>
        /// <param name="endIndex"></param>
        public Vertices(Vector2[] vector2, int startIndex, int endIndex)
        {
        ...
        /// <summary>
        /// build a vertex collection from a source Vertices colleciton.  if endindex is less thatn start, will be reversed.
        /// </summary>
        /// <param name="vector2"></param>
        /// <param name="startIndex"></param>
        /// <param name="endIndex"></param>
        public Vertices(Vertices vector2, int startIndex, int endIndex)
        {
        ...
        public Vertices(List<Vector2> vertices):base(vertices)
        {
        ...
        /// <summary>
        /// Gets the area.  works for any polygon not crossing itself, concave  or convex.  
        /// </summary>
        /// <returns></returns>
        ...
        {
            float area;
            return GetCentroid(out area);
        }

	//shadowplay mod add area,  used by bouyancy to get this all at once.

	/// <summary>
	/// Gets the centroid and the area.  NOTE, far from origin, doesnt work
	/// <param name="area">returns the area</param>
	/// <returns>centroid</returns>
	public Vector2 GetCentroid( out float area)
        {
        ...
        /// <summary>
        /// fix for centroid of wcs verts of bodies far from origin
        /// </summary>
        /// <param name="body"></param>
        /// <param name="area"></param>
        /// <param name="pts"></param>
        /// <returns></returns>
        public static Vector2 GetCentroidLargeValues(FarseerPhysics.Dynamics.Body body, out float area,  Vertices pts)
        {
            Vector2 centroid;

            Vertices ptsSubmergedLocal = new Vertices();
            for (int i = pts.Count - 1; i >= 0; i--)   //reverse the Vector2s so its winding is counterclockwise
            {
                ptsSubmergedLocal.Add(pts[i] - body.Position);
            }

            centroid = ptsSubmergedLocal.GetCentroid(out area);  //this expects counterclockwise.
            return centroid + body.Position;
        }
        ...
        /// <summary>
        /// Returns an AABB for a set of vertices, while negating the Y for normal clockwise  Cartesian coordinates, pos vectors are in quadrant I.  
        /// Normally we use graphics CS throughout,  +Y goes down, so set to false,  then   the canvas lines  up with the model.. just scaled and offset. 
        ///   
        /// </summary>
        /// <returns></returns>
        public AABB GetCollisionBox(bool yisUp = true )
        {
            AABB aabb;
            ...
            if (yisUp)  //put it back to the fucked up way we are used to... don't know how many time s this has hurt..  not sure if if hurts the other way..
            {
                flipY(ref aabb.UpperBound);
                flipY(ref aabb.LowerBound);
            }
        ...
        /// <summary>
        /// Scales the vertices with the specified vector.   around the origin
        /// </summary>
        ...
        public void ScaleCenter(ref Vector2 value)
        {

            //TODO get centroid,  translate , scale, translate back
        }
        ...
        /// <summary>
        /// Path is wound conterclockwise,  shadowPlay note: since we are using graphics coordinates , a counterclockwise path will appear clockwise on the screen.
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Check for edge crossings..  slow method..  IsSelfIntersecting is be faster.  both seem to work
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// reports in o(n) time if polygon is not simple  .. 
        /// </summary>
        /// <returns></returns>
        public bool IsSelfIntersecting()
        {

            if (Count == 3) //dh ShadowPlay Mods, was giving wroing results cutting a triangle, but is impossible for 3 verts to self intersect
                return false;
            ...
            for (int i = 0; i < Count; ++i)
            {
                if (i < Count - 1)
                {
                    for (int h = i + 1; h < Count; ++h)
                    {
                        ...
                        if (this[i] == this[h])
                        {
                            Debug.WriteLine("IsSelfIntersecting verts are on each other");
                            return true;
                        }
                    }
                }
        ...
        /// 
        //TODO pass in allowed cases.. 
        public int CheckPolygon() //Shadowplay Mod, partial merge from latest, return the raw error code
        {
            int error = -1;
        ...
        /// <summary>
        /// like getCollision box but only x and takes fixed buffer vertex
        /// </summary>
        /// <param name="vertices"></param>
        /// <param name="min"></param>
        /// <param name="max"></param>
        public  static void GetXBounds(Vector2[] vertices, out float min, out float max)
        {
             min = float.MaxValue;     
             max = float.MinValue;
        ...
        /// <summary>
        /// Translate vertices position so their centroid is close to (0,0)..  this is for floating point issues that happen when polygon is miles from home
        /// </summary>
        public void Centralize()
        {
            Vector2 center = GetCentroid();
            center = -center;
            Translate(ref center);
        }
           ...
           /// <summary>
        /// Checks if polygon is valid for use in Box2d engine, to be tesselated.. used in Complex object cut.
        /// Last ditch effort to ensure no invalid polygons are
        /// added to world geometry.
        ///
        /// Performs a full check, for simplicity, convexity,
        ...
        /// 
        //TODO pass in allowed cases.. 
        public int CheckConcavePolygon() //Shadowplay Mod, partial merge from latest, return the raw error code
        {
            int error = -1;
            if (Count < 3 || Count > Settings.MaxPolygonVertices)
            {
