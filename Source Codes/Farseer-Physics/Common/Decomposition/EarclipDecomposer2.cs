            ...
            while (vNum > 3)
            {
                ...
                for (int i = 0; i < vNum; ++i)
                {
                    if (IsEar(i, xrem, yrem, vNum))
                    {
                        ...
                        Vector2 d1 = Vector2.Normalize(new Vector2(xrem[upper] - xrem[i], yrem[upper] - yrem[i]));
                        Vector2 d2 = Vector2.Normalize(new Vector2(xrem[i] - xrem[lower], yrem[i] - yrem[lower]));
                        Vector2 d3 = Vector2.Normalize(new Vector2(xrem[lower] - xrem[upper], yrem[lower] - yrem[upper]));
