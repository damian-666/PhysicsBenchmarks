...
//using Matrix = Farseer.Xna.Framework.Matrix;

//using MathHelper = Farseer.Xna.Framework.MathHelper;



//TODO moved this to data from game.

//change   it bakc comment out the sprite use of vi

//SeekOrigin what vi is forr or keep going . fix ton of refs and to vector.. since rectagel will not be in 
//or geekp going moveing rect .. taking it out of the api effecively using it with farser vector 2..
//in forture MABYE  vector2 will be mcsft xna one.. copy body template to body and creathe the joint graph on load, it might not be as efficient tho.
// go back to nez.. 

// see if this all make snexse to reorg code like this taking nez into 3 places
//if its right only do it once.. do we really even need srite wiht rv .. 
//check if to simplify sprite first.
//if va not needed ..if sprite font not..lets simplfiy
//and   strippling down cod eis alwahys best.
//remove rest of MONOTOUCH again..

namespace Core.Data.Geometry
{
	/// <summary>
	/// Describes a 2D-rectangle.   Note:  this class exists in Netstandard very similar meaning and implentation, so repeat code basically here
	///But for now We are using the version a s found in Nez just because it
	/// was tested in Nes methods.
	/// </summary>
