#if !UNIVERSAL    //RIGHT now behaviors is a Keycollection that notifies.   we are not using    UndoRedoList<T> .   probably the best solution would be to wrap the collections in the controller.  have the collect add to it
//for example  Entities.   instead of write directly to it.   write to  this.. and get it to compile.  to do that, might need to move it out of a portable   
//universal module, because there are difference in CollectionNotificatoin in Wpf and SL.. one day might want to edit in game code, or merge stuff.

//undo adding stuff like  emitters, entities, etc would be nice.   See how grips are implemented..
...
namespace UndoRedoFramework.Collections.Generic
{
    ...
    /// <summary>
    /// This was taken from CodeProject, was a nice idea for the UI this just as prop pages to go directly to the model.   its has very limited overhead and not used in game, just in tool design mode.
    /// however each property must be coded with a template.. probably best to use this in a view model layer.. especially since very few of data in 
    /// Body and Spirit are directly undoable now.  The vertices collection on the control for Body GeneralVertices is the most ( if not only) useful one of these. TODO .. put on color and some other setters.  the prop sheet changes do not start and finish and undo operaton, TODO ( maybe they could) .. select tools do using Command
    /// http://www.codeproject.com/Articles/19550/Automated-Undo-Redo-library-Csharp-Generics-NET-C
    /// the downside is the model is cluttered with this stuff and if diverges from the farseer even more.. its pretty ugly code.. useful for A/B in tuning..  
    /// </summary>  
    /// <typeparam name="T"></typeparam>
    public class UndoRedoList<T> : IList<T>
#if !(UNIVERSAL || SILVERLIGHT)
        , IUndoRedoMember 
        , ICollection<T>, IEnumerable<T>, IList, ICollection, IEnumerable  //TODO conditionally compile more code below.. 
#endif
    {
        List<T> list;
...
#region IUndoRedoMember Members
        ...
        {
            foreach (T x in list)// even if action modifies the list, the changes will be caught by appropriate changing member
            {
                action(x);
            }
        }
