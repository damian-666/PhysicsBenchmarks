...
using Vector2 = Farseer.Xna.Framework.Vector2;  //because Vecto2 is persistent type used for serialization of Body in level.. TODO design weakness.. maybe alias it somehow
...
namespace FarseerPhysicsView
{
    ...
    public class DebugView : DebugViewBase, IDisposable
    {
        ...
        private void DrawDebugPanel()
        {
            ...
            _debugPanelSbObjects.Append("- Controllers: ").AppendNumber(World.Controllers.Count).AppendLine();
            DrawString(x, y, _debugPanelSbObjects);
            
            _debugPanelSbUpdate.Clear();
            _debugPanelSbUpdate.Append("Update time:").AppendLine();
            _debugPanelSbUpdate.Append("- Body:    ").AppendNumber(  (float)World.SolveUpdateTime, 3).Append(" ms").AppendLine();
            _debugPanelSbUpdate.Append("- Contact: ").AppendNumber(  (float)World.ContactsUpdateTime, 3).Append(" ms").AppendLine();
            _debugPanelSbUpdate.Append("- CCD:     ").AppendNumber(  (float)World.ContinuousPhysicsTime, 3).Append(" ms").AppendLine();
            _debugPanelSbUpdate.Append("- Joint:   ").AppendNumber(  (float)World.Island.JointUpdateTime, 3).Append(" ms").AppendLine();
            _debugPanelSbUpdate.Append("- Controller:").AppendNumber((float)World.ControllersUpdateTime, 3).Append(" ms").AppendLine();
            _debugPanelSbUpdate.Append("- Total:   ").AppendNumber(  (float)World.UpdateTime, 3).Append(" ms").AppendLine();
